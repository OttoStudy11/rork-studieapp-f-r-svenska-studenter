...
export default function Fysik2() {
  ...
  const storageKey = `@fysik2_progress_${user?.id}`;
    ...
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
          ...
          <View>
            <LinearGradient colors={['#F59E0B', '#D97706']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.heroCard}>
              <View style={styles.heroContent}>
                ...
                <Text style={styles.heroTitle}>Fysik 2</Text>
                <Text style={styles.heroDescription}>Elektricitet, magnetism och vågor</Text>
              </View>
        ...
        <FadeInView delay={150}>
          <TouchableOpacity style={[styles.flashcardsButton, { backgroundColor: theme.colors.card }]} onPress={() => router.push('/flashcards/FYSFYS02')} activeOpacity={0.7}>
        ...
        <FadeInView delay={200}>
          <View style={[styles.introSection, { backgroundColor: theme.colors.card }]}>
            ...
            <Text style={[styles.introText, { color: theme.colors.textSecondary }]}>Fysik 2 fokuserar på elektromagnetism, vågrörelselära och moderna fysikens grunder.</Text>
          </View>
        </FadeInView>
              ...
              <TouchableOpacity style={[styles.moduleCard, { backgroundColor: theme.colors.card }, module.completed && styles.moduleCardCompleted]} onPress={() => setExpandedModule(expandedModule === module.id ? null : module.id)} activeOpacity={0.7}>
                <View style={styles.moduleHeader}>
                  <TouchableOpacity style={styles.checkboxContainer} onPress={(e) => { e.stopPropagation(); toggleModuleCompletion(module.id); }}>
                    {module.completed ? <CheckCircle size={24} color="#F59E0B" /> : <Circle size={24} color={theme.colors.textMuted} />}
                  </TouchableOpacity>
                  <Text style={styles.moduleEmoji}>{module.emoji}</Text>
                  <View style={styles.moduleTitleContainer}>
                    <Text style={[styles.moduleTitle, { color: theme.colors.text }, module.completed && { color: '#F59E0B' }]}>Modul {module.id}: {module.title}</Text>
