-- Complete Study Groups System Setup
-- This SQL file creates the complete study groups functionality with proper type handling

-- First, drop existing tables if they exist to ensure clean setup
DROP TABLE IF EXISTS study_group_session_participants CASCADE;
...
DROP FUNCTION IF EXISTS add_group_creator_as_owner() CASCADE;
DROP FUNCTION IF EXISTS update_study_groups_updated_at() CASCADE;
DROP FUNCTION IF EXISTS update_study_group_messages_updated_at() CASCADE;
DROP FUNCTION IF EXISTS update_study_group_sessions_updated_at() CASCADE;
...
-- Create study group sessions table (scheduled group study sessions)
...
CREATE TABLE study_group_session_participants (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_study_group_sessions_group ON study_group_sessions(group_id);
CREATE INDEX idx_study_group_sessions_start ON study_group_sessions(scheduled_start);
CREATE INDEX idx_study_group_session_participants_session ON study_group_session_participants(session_id);
CREATE INDEX idx_study_group_session_participants_user ON study_group_session_participants(user_id);
...
-- Insert some example groups for testing (optional)
-- Uncomment if you want to add example data
/*
INSERT INTO study_groups (name, description, created_by, is_private)
VALUES 
  ('Matematik 1a Studiegrupp', 'Pluggar tillsammans inför nationella prov', 
   (SELECT id FROM profiles LIMIT 1), false),
  ('Svenska B Gruppstudier', 'Hjälper varandra med uppsatser och läsförståelse', 
   (SELECT id FROM profiles LIMIT 1), false);
*/
