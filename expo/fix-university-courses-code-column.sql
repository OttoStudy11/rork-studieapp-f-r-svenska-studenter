-- Fix university_courses table - add missing code column
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_name = 'university_courses' AND column_name = 'code'
  ) THEN
    ALTER TABLE university_courses ADD COLUMN code TEXT;
    RAISE NOTICE 'Added code column to university_courses';
  END IF;
END $$;

-- Update the code column with values from id where code is null
UPDATE university_courses SET code = id WHERE code IS NULL;

-- Make code NOT NULL after populating
ALTER TABLE university_courses ALTER COLUMN code SET NOT NULL;

-- Create index for code lookups
CREATE INDEX IF NOT EXISTS idx_university_courses_code ON university_courses(code);

-- Now insert/update all courses
-- =====================================================
...
-- =====================================================
INSERT INTO university_courses (id, code, name, credits, year, mandatory, category, field, program_id, description) VALUES
('FO1001', 'FO1001', 'Förskolan i samhället', 15, 1, true, 'professionskurs', 'Utbildningsvetenskap', 'forskollararprogrammet', 'Förskolans uppdrag och historia'),
('FO1002', 'FO1002', 'Barns utveckling och lärande', 15, 1, true, 'professionskurs', 'Utbildningsvetenskap', 'forskollararprogrammet', 'Utvecklingspsykologi för förskolan'),
('FO1003', 'FO1003', 'Verksamhetsförlagd utbildning 1', 15, 1, true, 'professionskurs', 'Utbildningsvetenskap', 'forskollararprogrammet', 'Praktik i förskolan'),
('FO1004', 'FO1004', 'Pedagogiskt ledarskap', 15, 1, true, 'professionskurs', 'Utbildningsvetenskap', 'forskollararprogrammet', 'Ledarskap i pedagogisk verksamhet'),
('FO2001', 'FO2001', 'Språk och kommunikation', 15, 2, true, 'professionskurs', 'Utbildningsvetenskap', 'forskollararprogrammet', 'Språkutveckling hos barn'),
('FO2002', 'FO2002', 'Matematik och naturvetenskap', 15, 2, true, 'professionskurs', 'Utbildningsvetenskap', 'forskollararprogrammet', 'Matematiskt och naturvetenskapligt lärande'),
('FO2003', 'FO2003', 'Skapande verksamhet', 15, 2, true, 'professionskurs', 'Utbildningsvetenskap', 'forskollararprogrammet', 'Estetiska uttrycksformer'),
('FO2004', 'FO2004', 'Verksamhetsförlagd utbildning 2', 15, 2, true, 'professionskurs', 'Utbildningsvetenskap', 'forskollararprogrammet', 'Fördjupad praktik'),
('FO3001', 'FO3001', 'Specialpedagogik', 15, 3, true, 'professionskurs', 'Utbildningsvetenskap', 'forskollararprogrammet', 'Barn med särskilda behov'),
('FO3002', 'FO3002', 'Verksamhetsförlagd utbildning 3', 22.5, 3, true, 'professionskurs', 'Utbildningsvetenskap', 'forskollararprogrammet', 'Avslutande praktik'),
('FO3003', 'FO3003', 'Examensarbete', 15, 3, true, 'professionskurs', 'Utbildningsvetenskap', 'forskollararprogrammet', 'Självständigt vetenskapligt arbete')
ON CONFLICT (id) DO UPDATE SET
  code = EXCLUDED.code,
  name = EXCLUDED.name,
  credits = EXCLUDED.credits,
...
('WEB105', 'WEB105', 'Databaser', 20, 1, true, 'grundkurs', 'Webbutveckling', 'webbutvecklare', 'Databashantering'),
