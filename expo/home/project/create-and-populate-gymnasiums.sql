-- =====================================================
-- SKAPA OCH FYLL GYMNASIUMS TABELL
-- =====================================================
-- Skapar gymnasiums tabell och fyller den med alla
-- gymnasieskolor i Sverige från Skolverkets data
-- =====================================================

BEGIN;

-- Radera befintlig tabell om den finns
DROP TABLE IF EXISTS gymnasiums CASCADE;

-- Skapa gymnasiums tabell
...
CREATE INDEX idx_gymnasiums_city ON gymnasiums(city);
CREATE INDEX idx_gymnasiums_type ON gymnasiums(type);
CREATE INDEX idx_gymnasiums_name ON gymnasiums(name);

-- Aktivera RLS
ALTER TABLE gymnasiums ENABLE ROW LEVEL SECURITY;

-- Policy för att alla kan läsa gymnasiums
CREATE POLICY "Anyone can view gymnasiums"
  ON gymnasiums FOR SELECT
  USING (true);
