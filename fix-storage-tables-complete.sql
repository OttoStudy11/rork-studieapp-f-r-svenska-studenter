-- Complete fix for storage tables with correct types
-- Run this SQL in your Supabase SQL editor

-- =====================================================
-- FIX COURSES TABLE ID TYPE (IF NEEDED)
-- =====================================================
-- Check if courses.id is UUID or TEXT and ensure consistency

DO $$ 
BEGIN
  -- Check if courses table exists and what type id column is
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'courses') THEN
    RAISE NOTICE 'Courses table exists, checking id column type...';
  ELSE
...
-- =====================================================
-- DROP AND RECREATE ACTIVE_TIMER_SESSIONS WITH CORRECT TYPES
-- =====================================================
-- Drop the table if it exists (will also drop dependent objects)
DROP TABLE IF EXISTS active_timer_sessions CASCADE;

-- Create with course_id as TEXT to match courses.id
...
-- =====================================================
-- CREATE/UPDATE USER_ONBOARDING TABLE
-- =====================================================
...
-- Drop old policies if they exist
DROP POLICY IF EXISTS "Users can view their own onboarding status" ON user_onboarding;
DROP POLICY IF EXISTS "Users can insert their own onboarding status" ON user_onboarding;
DROP POLICY IF EXISTS "Users can update their own onboarding status" ON user_onboarding;
DROP POLICY IF EXISTS "Users can delete their own onboarding status" ON user_onboarding;
...
CREATE INDEX IF NOT EXISTS idx_active_timer_sessions_course_id ON active_timer_sessions(course_id);
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'active_timer_sessions') THEN
    RAISE NOTICE '✅ active_timer_sessions table created successfully';
  END IF;
