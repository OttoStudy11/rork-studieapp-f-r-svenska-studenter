-- Fix course content insertion errors
-- This script fixes the ON CONFLICT issues and duplicate tag errors

-- First, let's check if we need to add unique constraints or handle conflicts differently
-- Since courses table doesn't have unique constraint on title, we'll use different approach
...
BEGIN
    ...
    SELECT id INTO svenska2_course_id FROM courses WHERE title = 'Svenska 2' AND subject = 'Svenska' LIMIT 1;
...
- Kreativt skrivande', 
    ...
    SELECT id INTO tag_grammatik_id FROM course_content_tags WHERE name = 'Grammatik' LIMIT 1;
...
BEGIN
    -- Check if course already exists, if not create it
    SELECT id INTO svenska3_course_id FROM courses WHERE title = 'Svenska 3' AND subject = 'Svenska' LIMIT 1;
    
    IF svenska3_course_id IS NULL THEN
        svenska3_course_id := gen_random_uuid();
        INSERT INTO courses (id, title, subject, level, description) VALUES
...
- Miljöbeskrivning', 
    ...
    -- Check if tag already exists
    SELECT id INTO tag_litteratur_id FROM course_content_tags WHERE name = 'Litteratur' LIMIT 1;
...
BEGIN
    -- Check if course already exists, if not create it
    SELECT id INTO engelska5_course_id FROM courses WHERE title = 'Engelska 5' AND subject = 'Engelska' LIMIT 1;
...
- Themes', 'theory', 1, 45, ARRAY['Analysera noveller', 'Känna till litterära begrepp'])
    ...
    -- Check if tag already exists (reuse Grammar tag if exists)
    SELECT id INTO tag_grammar_id FROM course_content_tags WHERE name = 'Grammar' LIMIT 1;
...
BEGIN
    -- Check if course already exists, if not create it
    SELECT id INTO engelska6_course_id FROM courses WHERE title = 'Engelska 6' AND subject = 'Engelska' LIMIT 1;
    
    IF engelska6_course_id IS NULL THEN
        engelska6_course_id := gen_random_uuid();
        INSERT INTO courses (id, title, subject, level, description) VALUES
...
- Imagery', 'theory', 1, 50, ARRAY['Kunna analysera dikter', 'Diskutera poetiska uttryck'])
    ...
    -- Check if tag already exists (reuse Literature tag if exists)
    SELECT id INTO tag_literature_id FROM course_content_tags WHERE name = 'Literature' LIMIT 1;
...
BEGIN
    -- Check if course already exists, if not create it
    SELECT id INTO matematik1_course_id FROM courses WHERE title = 'Matematik 1a/1b' AND subject = 'Matematik' LIMIT 1;
...
- Förenkling', 'theory', 1, 50, ARRAY['Kunna lösa enkla ekvationer', 'Förstå variabler'])
    ...
    -- Check if tag already exists
    SELECT id INTO tag_algebra_id FROM course_content_tags WHERE name = 'Algebra' LIMIT 1;
...
-- Success message
SELECT 'Course content insertion completed successfully!' as result;